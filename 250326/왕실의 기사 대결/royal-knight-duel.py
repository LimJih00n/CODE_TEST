...
#단위 test를 처음에 어떻게할지 고민해보자. case를 더 생각하는 연습을 해야할 거 같다. 초기 구현까지는 잘가나 이후 test case에서
문제가 생긴다. 
'''
import collections 
import copy
...
def check_can_move(knight,knights,dir_):
    ...
    main_knight = knight
    copy_knights = copy.deepcopy(knights)
    ...
    while queue:
        ...
        if cur_knight.set_new_p(cur_r+move_dir[dir_][0],cur_c+move_dir[dir_][1]): # 벽에 부딫히는지 확인 후 이동하기. 
            ...
            break
    if non_move: 
    # 다른게 부닥쳐서 이동을 안해야하는데 이동을 해버렸다. 이유는 움직임 자체가 그 값을 움직이고 있기 때문임
    # 복사해서 이동시키던가 해야한다....
        return copy_knights
